...
with XDMFFile(MPI.COMM_WORLD,
    ...
    xdmf.write_function(vm_p1_field)   # P1, lissé — pour Warp By Vector côté ParaView
    xdmf.write_function(vm_field)      # DG0, exact (von_mises_dg0) — pour valeurs non lissées,
                                        # non combinable avec Warp By Vector (géométrie séparée)
...
# VUE 4a : Courbes J_min, J_max dans le gap
if len(hist_steps) > 0:
    ...
    ax5.legend(facecolor="#0f0f23", labelcolor="white", fontsize=11)
