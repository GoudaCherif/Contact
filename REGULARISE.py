...
with XDMFFile(MPI.COMM_WORLD,
              os.path.join(save_dir, "mesh_cones_fillets.xdmf"), "r") as xdmf:
...
gamma     = 1e-4
mu_m      = 1.0
alpha_r   = 1.0    # raideur de régularisation (Wriggers, Korelc, Junker 2025,
                    # éq. 12/13). Le Tableau 1 du papier montre que pour
                    # gamma=1e-4 (notre valeur), c'est la plus PETITE valeur
                    # testée (alpha_r=1) qui converge en le moins d'itérations
                    # (65, vs 75 pour alpha_r=100) — alpha_r=100 n'est optimal
                    # que pour des gamma bien plus petits (1e-6, 1e-7). Notre
                    # blocage de progression avec alpha_r=100 est cohérent
                    # avec un régularisateur trop rigide pour ce gamma.
u_imposed = -1.7
n_steps   = 20
warp_factor = 1.7

#  Espace P2 (Lagrange degré 2) — requis par la régularisation (voir plus
# bas) : elle nécessite le gradient spatial de J et de tan(phi), qui sont
# des fonctions de F = I + grad(u). Avec P1, grad(u) est constant par
# élément, donc son propre gradient est nul p.p. — la régularisation
# n'aurait alors aucun effet. Avec P2, grad(u) est linéaire par élément et
# admet un gradient non nul et significatif.
...
# Espace P1 "miroir", UNIQUEMENT pour le post-traitement basé sur les
# indices de nœuds géométriques (masque de l'axe, point suivi, moyenne sur
# l'implant...). Ces sections du script indexent u_array par les indices
# de domain.geometry.x, ce qui suppose une correspondance 1-pour-1 entre
# DOFs et nœuds géométriques — vrai pour P1, FAUX pour P2 (qui ajoute des
# DOFs aux milieux des arêtes, absents de domain.geometry.x). Plutôt que
# de réécrire tout ce indexing, on interpole u (P2) dans cet espace P1 à
# chaque fois qu'on en a besoin : u_diag échantillonne alors exactement
# aux nœuds géométriques, sans aucune perte pour ces diagnostics (qui ne
# nécessitent pas la précision des DOFs milieu d'arête).
V_diag = fem.functionspace(domain, ("Lagrange", 1, (2,)))
u_diag = fem.Function(V_diag)
...
def W_third_medium(u, r, mu, gamma, alpha_r):
    # Wriggers, Korelc, Junker (2025), "A third medium approach for contact
    # using first and second order finite elements", éq. (4) + (13).
    #
    # W_m  : terme isochore pur (déjà présent avant régularisation) — nul
    #        pour toute dilatation isotrope, cf. discussion précédente.
    # W_reg: régularisation par gradient — pénalise les variations SPATIALES
    #        rapides de la rotation (tan phi) et du Jacobien J, ce qui
    #        contrôle la distorsion des éléments dans le gap sans imposer
    #        de résistance directe au changement de volume lui-même
    #        (contrairement à un terme en (J-1)^2). C'est le mécanisme
    #        documenté par le papier pour empêcher les éléments du
    #        troisième milieu de devenir dégénérés sous grande déformation.
    ...
    # Pour la régularisation (nécessite ufl.grad()), on reconstruit tan(phi)
    # et J directement à partir des dérivées de u, SANS passer par
    # ufl.as_matrix/indexation — contourne une limitation de la version UFL
    # système utilisée, où ufl.grad() n'arrive pas à déterminer la dimension
    # géométrique d'une expression obtenue par indexation d'une matrice
    # construite via as_matrix (ValueError: "Cannot determine geometric
    # dimension"). Formule strictement équivalente (F bloc-diagonal : le
    # déterminant du bloc r-z est indépendant de F_tt, qu'on multiplie
    # séparément).
    F00  = 1 + u[0].dx(0)
    F01  = u[0].dx(1)
    F10  = u[1].dx(0)
    F11  = 1 + u[1].dx(1)
    F_tt = 1 + u[0] / r
    J_reg   = F_tt * (F00 * F11 - F01 * F10)   # == ufl.det(F) ci-dessus
    tan_phi = (F01 - F10) / (F00 + F11)        # éq. 9 du papier

    grad_tan_phi = ufl.grad(tan_phi)
    grad_J       = ufl.grad(J_reg)
    W_reg = (alpha_r / 2) * (
        ufl.dot(grad_tan_phi, grad_tan_phi) +
        ufl.dot(grad_J, grad_J)
    )
    return gamma * (W_m + W_reg)
...
W_total = (
    ...
    W_third_medium(u, r, mu_m, gamma, alpha_r)  * r * dx(TAG_GAP)
)

F_form = ufl.derivative(W_total, u, v)
J_form = ufl.derivative(F_form, u, ufl.TrialFunction(V))
print("Formes variationnelles définies (régularisation Wriggers/Korelc/Junker, ∇tan(phi)+∇J).")
sys.stdout.flush()
...
while abs(current_u) < abs(u_imposed) and attempt_count < max_attempts:
    ...
    try:
        ...
        if reason > 0:
            ...
            # u_diag : shadow P1 échantillonnant u (P2) aux nœuds géométriques
            # -> permet d'indexer par cells_impl_nodes/node_top_axis (indices
            # géométriques), impossible directement sur u.x.array en P2.
            u_diag.interpolate(u)
            u_arr = u_diag.x.array.reshape(-1, 2)
            uz_min = u_arr[:,1].min()
...
#  Résumé final 
# u_diag : voir remarque plus haut — échantillonne u (P2) aux nœuds
# géométriques pour permettre tout l'indexing par coords/mask ci-dessous.
u_diag.interpolate(u)
u_array = u_diag.x.array.reshape(-1, 2)
...
#
# IMPORTANT : on dérive ici uniquement W_m (terme isochore, éq. 4), PAS le
# terme de régularisation par gradient W_reg (éq. 13, |∇tan(phi)|²+|∇J|²).
# Ce dernier dépend de ∇F (pas seulement de F ponctuellement) : sa contribution
# "naturelle" au champ de contrainte n'est pas une contrainte de Cauchy
# classique dW/dF, mais une contrainte d'ordre supérieur associée au terme
# de gradient (nécessiterait une formulation de type second gradient, avec
# ses propres conditions aux limites naturelles) — hors du cadre d'un simple
# post-traitement Von Mises. Ce que la contrainte affichée représente donc :
# la contrainte physique "classique" associée au comportement matériau du
# gap, SANS la contribution du terme de stabilisation numérique — cohérent
# avec l'esprit du papier, où la régularisation contrôle la forme des
# éléments sans être censée représenter une contrainte physique du matériau.
F_gap  = deformation_gradient_axi(u, r)
Fv     = ufl.variable(F_gap)
Cg     = Fv.T * Fv
Ic_g   = ufl.tr(Cg)
Jg     = ufl.det(Fv)
W_m_g  = gamma * (mu_m / 2) * (Jg**(-2/3) * Ic_g - 3)
P_gap  = ufl.diff(W_m_g, Fv)
...
with XDMFFile(MPI.COMM_WORLD,
              os.path.join(save_dir, "resultats_regularises_fillets.xdmf"), "w") as xdmf:
...
print("Fichier XDMF avancé sauvegardé : resultats_regularises_fillets.xdmf")
...
grid_def.save(os.path.join(save_dir, "resultats_deformes_von_mises_regularises.vtu"))
print("Maillage déformé (Von Mises exact) sauvegardé : resultats_deformes_von_mises_regularises.vtu")
...
if len(hist_steps) > 0:
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(save_dir, "jacobien_gap_regularise.png"), dpi=150,
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(save_dir, "descente_implant_regularise.png"), dpi=150,
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(save_dir, "descente_point_haut_implant_regularise.png"), dpi=150,
                facecolor=fig2.get_facecolor())
    plt.show()
